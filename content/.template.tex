\danceName{THIS IS A TEMPLATE}{longDMin}%danceName must be followed by blank line for proper command parsing
% parameters Dance name - index under which to list the dance. List of indexes
%can be easily seen in backmatter/indices.tex, in case a new index is required, must be added both to that file
%and misc/options.sty. Index option may be repeated as much as required, command takes any number of arguments
...
\begin{music}
    ...
    \nobarnumbers%
    \generalmeter{\allabreve}%  4/4 meter (C meter). Replace with \meterfracXY for X over Y meter
...
\begin{multicols}{2}
    This is a template original dance description. It cu{\s}tomary u{\s}e{\s} long S whereever it appears in original text, and \Pfa, resp \Pfb to mark the repetition symbols from original texts
    \columnbreak
